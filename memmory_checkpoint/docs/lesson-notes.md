...
## Lesson 7 — Long-Term Memory: Classification, Storage, Retrieval

### Key concepts learned
- **Long-term memory as durable, user-scoped, cross-thread fact storage** — the deliberate gap Lessons 3–6 left open. Everything before this lesson (checkpoints, summaries) was thread-scoped by design; `long_term_memories` is the first thing keyed on `user_id` alone.
- **Structured output, for the first time this course** — every prior LLM call (Lessons 5–6) returned free text taken as-is. `with_structured_output(MemoryClassification)` returns a validated Pydantic instance instead — the production-standard replacement for "ask the model for JSON and hope," since the schema is enforced by the model call itself, not parsed after the fact.
- **Classification as a judgment filter, not a save-everything pipe** — `'ignore'` is a first-class, expected-majority category. A memory store that saves every exchange isn't memory, it's a second `messages` table with extra steps.
- **FastAPI `BackgroundTasks`, verified against source, not assumed** — it's in-process, same event loop, best-effort — not a durable queue. Confirmed against FastAPI's own docs: a dependency-with-yield's teardown code (closing the request's DB session) runs BEFORE background tasks execute, not after. A background task must open its own fresh session; reusing the request's is a guaranteed failure, not a maybe.
- **The Lesson 6 checkpoint-scoping fork resurfaces in a new place** — `'manual'`/`'langmem_function'` rebuild their system message from Postgres every turn (memories always fresh, including mid-thread updates). `'langmem_node'` can only inject memories once, at thread creation, because the checkpoint accumulates via `add_messages` rather than rebuilding — verified against LangMem's own API reference that a leading system message is pinned and excluded from summarization, which is exactly what makes "inject once" both possible and safe.
- **Derived data vs. terminal data** (crystallized in 7.9's explain-back) — a summary is recomputed fresh from the untouched `messages` table every time it triggers, so a wrong summary just gets regenerated; a long-term memory, once written, has no automatic path back to the source exchange — it's the terminal record from that moment on, which is exactly why it needs `source_thread_id` (traceability) and soft-delete (the only correction mechanism) where a summary needed neither.
- **(Conceptual, not built) Solving `langmem_node`'s staleness in the StateGraph world** — discussed but deliberately not implemented: a dedicated node fetching memories fresh every turn, writing to a disposable non-`messages` state key (same pattern `SummarizationNode` already uses via `output_messages_key`), merged into the model call transiently rather than pinned into the permanent transcript.
- **(Conceptual) Where a vector DB actually fits** — changes *what* gets retrieved (a relevance-ranked subset), not *when*. Combining live per-turn retrieval with vector search solves both freshness and scale at once — the mature production version of long-term memory looks like RAG, not a static system-prompt snapshot.

### Important decisions & why
- **`Exchange` dataclass** (`user_message` + `assistant_message`), not two bare string params — the classifier needs both sides, since the assistant's own reply can be what confirms a preference, not just the user's raw message.
- **Column is `memory_type`, not `category`** — corrected against the actual `models.py`, not the tracker's looser wording, before writing any code against it.
- **`DEFAULT_CONFIDENCE = Decimal("0.80")` hardcoded, explicitly flagged as a placeholder** — nothing reads this value yet to filter storage or retrieval; the only real "aggressiveness" lever today is the classifier's own prompt.
- **`get_active_memories` selects `.content` directly**, not full ORM rows — narrow `list[str]` return type so context-assembly callers never need to know the table's schema.
- **`format_memories_block` centralized in `long_term_memory.py`**, imported by both summarizer modules and `routes_chat.py` — same "shared constants prevent drift" principle as `KEEP_RAW_COUNT`/`SUMMARIZE_TRIGGER_COUNT` from Lesson 5.
- **`active_memories` fetched once per `/chat` call**, not re-queried per branch — accepted minor inefficiency for `langmem_node` (only needs it on a thread's first turn) in favor of simpler, uniform code across all three modes.
- **`langmem_node`'s injection gated on `is_new_thread`** (via `graph.aget_state`), not resent every turn — resending would duplicate system messages in `add_messages`-accumulated state, turn after turn, forever.
- **Debug endpoints (7.5/7.6) built as full, unfiltered audit views**, deliberately different from `get_active_memories`'s narrow "what the model sees" query — same "long-term memory must never be a black box" philosophy as the Lesson 4–6 debug endpoints.
- **Soft-delete only, never `db.delete()`** — same reasoning as Lesson 1's no-cascade-deletes decision, reinforced by 7.9's derived-vs-terminal distinction: there's no automatic regeneration to fall back on, so the row itself is the only record of what happened and must survive.
- **7.6's ownership check collapses "doesn't exist" and "belongs to someone else" into the same 404** — deliberate; a distinct 403 would leak which memory IDs exist for other users via the response code alone.

### Bugs hit and fixes
1. **Device-bridge staging cache bug recurred a third time** (same as Lessons 5 & 6's bug #6) — `device_stage_files` reported fresh byte counts/mtimes consistent with real edits, but served stale content on read, across multiple files this lesson. Same workaround: direct file attachment to chat instead of stage-then-read.
2. **`import select` (stdlib) instead of `from sqlalchemy import select`** in `long_term_memory.py` — same exact class of bug as Lesson 6's `load_running_summary` typo, this time breaking `get_active_memories`. Caught via direct file review before marking 7.2/7.3 complete; fixed.
3. **`routes_chat.py`: `{"message": [...]}`** — singular key typo on `langmem_node`'s new-thread branch, silently not matching `AgentState`'s actual `messages` field. Would have meant the graph received no messages at all on a thread's first turn. Caught via direct file review; fixed.
4. **`langmem_summarizer.py`: `return [result.messages, *system_message]`** — two compounding bugs: `result.messages` (itself a list) left nested instead of flattened, and `*system_message` splatting a single Pydantic-backed message object — which doesn't raise an error, since Pydantic models support iteration into `(field_name, value)` pairs, so it silently produced garbage tuples instead of a message. A bug that wouldn't fail where it happened, only downstream when LangGraph tried to interpret the tuples as messages. Fixed to `[system_message, *result.messages]`.
5. **Unused `LongTermMemory` import left in `routes_chat.py`** — harmless, same stray-import pattern as prior lessons; flagged, not blocking.
6. **Duplicate long-term-memory rows during 7.7's live test** — traced to two separate `/chat` requests to the same thread (one stating name, one stating currency preference), not a background-task double-fire. Confirmed, not a bug.

### Commands used
No new `uv add` this lesson — `langchain-openai`'s `with_structured_output`, `pydantic`, and FastAPI's `BackgroundTasks` were all already available from Lesson 1's install. Testing via Postman against `POST /chat` (varying `summary_mode`), `GET /debug/users/{user_id}/memories`, `DELETE /debug/users/{user_id}/memories/{memory_id}`, and `GET /debug/threads/{thread_id}/summary`.

### Self-check / confirmation results
- **7.1–7.6 (implementation)**: each confirmed via direct review of the actual files on disk, not say-so — caught bugs #2–#5 above before any item was marked complete.
- **7.7 confirmed live**: full flow above, with `final_context` from the brand-new thread showing the injected facts block directly, and `/debug/threads/{id}/summary` ruling out a checkpoint/summary leak — proving the fact came exclusively from `long_term_memories`.
- **7.8 confirmed after two corrections**: context-bloat and conservative-failure reasoning correct from the start; corrected the `confidence=0.80`-as-active-dial misconception, and supplemented the aggressive-failure mode with its real severity (a permanent false belief injected into every future thread, not just wasted tokens) — tied directly to why 7.5/7.6 exist.
- **7.9 confirmed after one correction**: the derived-vs-terminal distinction and the "pollutes one thread vs. corrupts every future thread" framing were both correct and were the strongest part of the answer. The closing "why different fix mechanisms" reasoning initially reached for Lesson 6's checkpoint-vs-Postgres fork (a different distinction); corrected to derived/regeneratable vs. terminal/permanent.
